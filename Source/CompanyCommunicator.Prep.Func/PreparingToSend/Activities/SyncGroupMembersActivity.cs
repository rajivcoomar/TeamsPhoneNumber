    ...
    public class SyncGroupMembersActivity
    {
        ...
        [ActivityTrigger] (string notificationId, string groupId) input, ILogger log)
        {
            ...
            try
            {
                ...
                var notificationData = await this.notificationDataRepository.GetAsync(
                partitionKey: NotificationDataTableNames.SentNotificationsPartition,
                rowKey: notificationId);

                // Get all members.
                var users = await this.groupMembersService.GetGroupMembersAsync(groupId, notificationData.MessageType);

                // Convert to Recipients
                var recipients = await this.GetRecipientsAsync(notificationData, users);
        ...
        private async Task<IEnumerable<SentNotificationDataEntity>> GetRecipientsAsync(NotificationDataEntity notification, IEnumerable<User> users) //string notificationId
        {
            ...
            await Task.WhenAll(users.ForEachAsync(maxParallelism, async user =>
            {
                ...
                if (userEntity == null)
                {
                    userEntity = new UserDataEntity()
                    {
                        AadId = user.Id,
                        UserType = userType,
                        MobileNumber = user.MobilePhone ?? string.Empty,
                        PreferredLanguage = "English", // user.PreferredLanguage ?? string.Empty,
                        PresenceStatus = user.Presence?.Activity ?? string.Empty,
                    };
                }
                else
                {
                    ...
                    {
                        userEntity.MobileNumber = user.MobilePhone ?? string.Empty;
                    }
                }

                recipients.Add(userEntity.CreateInitialSentNotificationDataEntity(notification: notification));
            }));
