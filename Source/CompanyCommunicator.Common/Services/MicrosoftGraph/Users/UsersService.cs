...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Common.Services.MicrosoftGraph
{
    ...
    internal class UsersService : IUsersService
    {
        ...
        /// <inheritdoc/>
        public async Task<(IEnumerable<User>, string)> GetAllUsersAsync(string messageType, string deltaLink = null)
        {
            var users = new List<User>();
            ...
            if (string.IsNullOrEmpty(deltaLink))
            {
                collectionPage = await this.graphServiceClient
                    ...
                    .Delta()
                    .Request()
                    .Select("id, displayName, userPrincipalName, userType, mobilePhone, preferredLanguage")
            ...
            if (messageType == "TeamsthenSMS")
            {
                foreach (var usr in users)
                {
                    usr.Presence.Activity = await this.GetUserPresence(usr.Id);
                }
            }
        ...
        private async Task<string> GetUserPresence(string userId)
        {
            var presence = await this.graphServiceClient
                .Users[userId]
                .Presence
                .Request()
                .GetAsync();

            return presence?.Activity ?? string.Empty;
        }
