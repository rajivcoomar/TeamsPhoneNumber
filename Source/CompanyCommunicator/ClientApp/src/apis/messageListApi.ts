...
export const getFLAGroupAccess = async (): Promise<any> => {
    let url = baseAxiosUrl + "/groupData/getFLAGroupAccess";
    return await axios.get(url);
}

export const getFLAGroups = async (): Promise<any> => {
    let url = baseAxiosUrl + "/groupData/getFLAGroups";
    return await axios.get(url);
}
