		...
		const out = [{
			...
			content: (
				<Flex vAlign="center" fill gap="gap.small">
					...
					<Flex.Item size="size.quarter" variables={{ 'size.quarter': '13%' }}>
						<Text></Text>
					</Flex.Item>
					<Flex.Item size="size.quarter" variables={{ 'size.quarter': '13%' }} shrink={false}>
						<Text
							truncated
							content={this.localize("TeamsRecipients")}
							weight="bold"
						>
						</Text>
					</Flex.Item>
					<Flex.Item size="size.quarter" variables={{ 'size.quarter': '13%' }} shrink={false}>
						<Text
							truncated
							content={this.localize("SMSRecipients")}
							weight="bold"
						>
						</Text>
					</Flex.Item>
		...
		return (
			<Flex className="listContainer" vAlign="center" fill gap="gap.small">
				...
				<Flex.Item size="size.quarter" variables={{ 'size.quarter': '13%' }} shrink={false}>
					<div>
						<TooltipHost content={this.props.t("TooltipSuccess")} calloutProps={{ gapSpace: 0 }}>
							<AcceptIcon xSpacing="after" className="smsSucceeded" outline />
							<span className="semiBold">{formatNumber(message.smsSucceeded)}</span>
						</TooltipHost>
						<TooltipHost content={this.props.t("TooltipFailure")} calloutProps={{ gapSpace: 0 }}>
							<CloseIcon xSpacing="both" className="smsFailed" outline />
							<span className="semiBold">{formatNumber(message.smsFailed)}</span>
						</TooltipHost>
						{
							message.smsCanceled &&
							<TooltipHost content="Canceled" calloutProps={{ gapSpace: 0 }}>
								<ExclamationCircleIcon xSpacing="both" className="smsCanceled" outline />
								<span className="semiBold">{formatNumber(message.smsCanceled)}</span>
							</TooltipHost>
						}
						{
							message.smsUnknown &&
							<TooltipHost content="Unknown" calloutProps={{ gapSpace: 0 }}>
								<ExclamationTriangleIcon xSpacing="both" className="smsUnknown" outline />
								<span className="semiBold">{formatNumber(message.smsUnknown)}</span>
							</TooltipHost>
						}
					</div>
				</Flex.Item>
