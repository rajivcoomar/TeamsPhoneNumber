...
import { getDraftNotification, getTeams, createDraftNotification, updateDraftNotification, searchGroups, getGroups, verifyGroupAccess, getFLAGroupAccess, getFLAGroups } from '../../apis/messageListApi';
import {
	getInitAdaptiveCard, setCardTitle, setCardImageLink, setCardSummary,
	setCardAuthor, setCardBtn, hideCardImage
...
export interface formState {
	...
	selectedsendMessageType: string,
	isFLAGroupMember: boolean
...
class NewMessage extends React.Component<INewMessageProps, formState> {
	...
	constructor(props: INewMessageProps) {
		...
		this.state = {
			...
			selectedsendMessageType: "TeamsOnly"
		}
	...
	public setDefaultCard = (card: any) => {
		//const sendMessageTypeAsString = this.localize("TitleText");
		//alert(this.state != undefined ? this.state.sendMessageType : "");
		if (this.state != undefined && this.state.sendMessageType != "TeamsOnly") {
			...
			setCardTitle(this.card, titleAsString);
			setCardImageLink(this.card, "");
			setCardSummary(this.card, summaryAsString);
			setCardAuthor(this.card, authorAsString);
			setCardBtn(this.card, "", "");
		} else {
			const titleAsString = this.localize("TitleText");
			const summaryAsString = this.localize("Summary");
			const authorAsString = this.localize("Author1");
	...
	private checkFLAGroupAccess = async () => {
		try {
			debugger;
			const responseGrp = await getFLAGroups();

			const response = await getFLAGroupAccess();
			//alert(response);
			//alert(response.data);
			this.setState({
				isFLAGroupMember: response.data
			});
		} catch (error) {
			return error;
		}
	...
	private getItem = async (id: number) => {
		try {
			...
			//alert(draftMessageDetail.messageType);
			...
			this.setState({
				...
				selectedsendMessageType: draftMessageDetail.messageType,
			});
			...
			this.setState({
				...
				loader: false,
				sendMessageType: draftMessageDetail.messageType,//"TeamsOnly"				
			}, () => {
				this.updateCard();
			});

			if (draftMessageDetail.messageType != "TeamsOnly") {
				//const titleAsString = this.localize("TitleText");
				//const summaryAsString = this.localize("Summary");
				//const authorAsString = this.localize("Author1");
				...
				//this.updateCard();
				document.getElementsByClassName("flexImgUrl")[0].style.display = "none";
				document.getElementsByClassName("ipBtnTitle")[0].style.display = "none";
				document.getElementsByClassName("ipBtnLnk")[0].style.display = "none";
			} else {
				//this.setDefaultCard(this.card);
				//this.updateCard();
				document.getElementsByClassName("flexImgUrl")[0].style.display = "flex";
				document.getElementsByClassName("ipBtnTitle")[0].style.display = "flex";
				document.getElementsByClassName("ipBtnLnk")[0].style.display = "flex";
			}
								...
								<Flex.Item size="size.half">
									<Flex column className="formContentContainer">
										<Flex gap="gap.small" vAlign="end">
											...
											<RadioGroup
												className="radioBtns radioBtnSndMsgType"
												checkedValue={this.state.selectedsendMessageType}
												onCheckedValueChange={this.onRadioOptChanged}
												vertical={false}
												items={[
													{
														name: "TeamsOnly",
														key: "TeamsOnly",
														value: "TeamsOnly",
														label: this.localize("TeamsOnly"),
													},
													{
														name: "SMSOnly",
														key: "SMSOnly",
														value: "SMSOnly",
														label: this.localize("SMSOnly"),
													},
													{
														name: "Both",
														key: "Both",
														value: "Both",
														label: this.localize("Both"),
													},
													{
														name: "TeamsthenSMS",
														key: "TeamsthenSMS",
														value: "TeamsthenSMS",
														label: this.localize("TeamsthenSMS"),
													}
												]}
											>
											</RadioGroup>
										...
										<Flex gap="gap.small" vAlign="end" className="flexImgUrl">
										...
										<Input className="inputField"
											...
											fluid
										/>
										<Input className="inputField ipBtnTitle"
											fluid
											...
											autoComplete="off"
										/>
										<Input className="inputField ipBtnLnk"
											fluid
										...
										<RadioGroup
											...
											items={[
												...
												//},
												{
													...
													children: (Component, { name, ...props }) => {
														return (
															<Flex key={name} column className="rdoBtnEveryone">
																<Component {...props} />
												...
												},
												{
													...
													children: (Component, { name, ...props }) => {
														return (
															<Flex key={name} column className="rdoBtnGroup">
																<Component {...props} />
	...
	private onSave = () => {
		...
		const draftMessage: IDraftMessage = {
			...
			messageType: this.state.sendMessageType,
		};
	...
	private onNext = (event: any) => {
		...
		this.setState({
			...
			this.updateCard();
			if (this.state != undefined && !this.state.isFLAGroupMember) {
				document.getElementsByClassName("rdoBtnEveryone")[0].style.display = "none";
				//document.getElementsByClassName("rdoBtnGroup")[0].checked = true;
			}
		});
	}
	...
	private onBack = (event: any) => {
		...
		this.setState({
			...
			// alert('back');
			this.updateCard();
			this.displayUIControls(this.state.sendMessageType);
		});
	}
	...
	private onRadioOptChanged = (event: any, data: any) => {
		//alert(data.value);

		this.setState({
			selectedsendMessageType: data.value,
			sendMessageType: data.value,
			title: "",
			summary: "",
			btnLink: "",
			imageLink: "",
		...
		this.displayUIControls(data.value);

	}

	private displayUIControls = (sendMessageType: string) => {
		if (sendMessageType != "TeamsOnly") {
			//const titleAsString = this.localize("TitleText");
			//const summaryAsString = this.localize("Summary");
			//const authorAsString = this.localize("Author1");
